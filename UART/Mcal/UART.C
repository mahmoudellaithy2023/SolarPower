...
void UART_Init(const UART_Config  ConfigCont, uint32  Baud_Rate)
{
    ...
    // Second Step is Enbaling Reciever and Transmitter In UART
            ...
            case ParityBit_ON:
                ...
                switch (StopBitsNum)
                {
                    case OneStop_Bit:
                        ...
                        #elif(ParityMode== OddParity_Mode)
                        {
...
uint8 UART_Receive(void)
{
    ...
    while ( !(UCSRA & (LOGICAL_ONE << RXC)) )
    {
        ...
        This indicates that There are Unread Data in the Recieve Buffer
        */
    }


    /*
    Checking if the frame Recieved has an errors or not 
    */
...
{
	uint8 i = 0;
	uint8 timeout = 2000; // Adjust the timeout value as needed
