...
/* Output     ! Nothing                                                                */
/***************************************************************************************/
void LCD_WriteCharctr(const LCD_config_T LCD_config, uint8 uint8DataCpy)
{
...
void LCD_WriteString(const LCD_config_T LCD_config, uint8* puint8StringCpy) {
	...
	while (puint8StringCpy[uint8Index] != '\0') {
		...
		LCD_WriteCharctr(LCD_config, puint8StringCpy[uint8Index]);
...
void LCD_WriteNumber(const LCD_config_T LCD_config, uint32 Number)
{
	...
	if(0 == Number)
	{
		LCD_WriteCharctr(LCD_Config, Number + 48);
	}
	...
	else if(0 != Number)
	{
		...
		{
			counter--;
			LCD_WriteCharctr(LCD_Config, (number_array[counter] + 48));
